...
| POST | `/api/claude/sesion` | Recibe evento hook Stop y guarda cada respuesta recibida |
...
| GET | `/api/claude/sesiones/{session_id}` | Detalle de respuestas individuales de una sesión |
| DELETE | `/api/claude/sesiones/{session_id}` | Elimina sesión |

## Gotcha Claude Code

No asumir idempotencia por `session_id` en `POST /api/claude/sesion`: se quitó el `UNIQUE` para poder guardar respuestas individuales dentro de una misma sesión. Para agregados, usar las consultas que agrupan por `session_id`.
